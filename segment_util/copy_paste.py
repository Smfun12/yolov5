...
def main(args):
    ...
    PATH_MIRROR = os.path.join(args.img_dir, 'train2012')
    label_path = os.path.join('/Users/Oleksandr_Reshetar/PycharmProjects/datasets/VOC2012/Annotations')
    label_folder = '/Users/Oleksandr_Reshetar/PycharmProjects/datasets/VOC/labels/train2012/'
    labels_in_folder = [i for i in os.listdir(label_folder) if i.endswith('.txt')]
    labels_in_folder.sort()
    # create output path
    ...
    for k, mask_path in enumerate(tbar):
        ...
        jpg_main_img = os.path.join(JPEGs, mask_main_path.replace('.png', '.jpg'))
        if not os.path.exists(jpg_main_img):
            jpg_main_img = os.path.join(PATH_MIRROR, mask_main_path.replace('.png', '.jpg'))
        if not os.path.exists(jpg_main_img):
            continue
        img_main = cv2.imread(jpg_main_img)

        if img_main is None or img_src is None:
            continue
        ...
        # cv2.imshow('win', mask)
        # cv2.waitKey()
        labels_in_img = util.parse_annotations_and_return_true_boxes(os.path.join(label_path, mask_path.replace('.png', '.xml')), img_main)
        src_lbl = np.zeros((len(labels_in_img)+1, 5))
        for it,i in enumerate(labels_in_img):
            src_lbl[it] = np.insert(i['bbox'], 0, i['name'])
        # src_lbl = np.loadtxt(lbl)
        lbl = str(int(labels_in_folder[-1].split('.')[0]) + 1) + '.txt'
        np.savetxt(os.path.join(label_folder, lbl), X=src_lbl)


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--input_dir", default="/Users/Oleksandr_Reshetar/PycharmProjects/datasets/VOC2012", type=str,
                        help="input annotated directory")
    parser.add_argument("--output_dir", default="/Users/Oleksandr_Reshetar/PycharmProjects/datasets/VOC2012_copy_paste", type=str,
                        help="output dataset directory")
    parser.add_argument("--img_dir", default="/Users/Oleksandr_Reshetar/PycharmProjects/datasets/VOC/images", type=str)
